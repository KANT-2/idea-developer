...
class TemplateContractTests(SimpleTestCase):
    ...
    def test_brainstorm_app_polls_incrementally_and_full_syncs_on_reconnect(self):
        ...
        self.assertIn("function applyEventBatch(data)", source)
        self.assertIn('event.target_type === "node"', source)
        self.assertIn('event.target_type === "connection"', source)
        self.assertIn('event.target_type === "canvas"', source)
        self.assertIn("if (data.has_more) return poll()", source)
        self.assertIn("function applyConfirmedNode(snapshot)", source)
        self.assertIn("data.node_type && applyConfirmedNode(data)", source)
        ...
        self.assertIn("nodeA.section_id !== null || nodeB.section_id !== null", source)
        self.assertIn('className: "brain-connection unclassified"', source)
        ...
        self.assertIn("function nextPollDelay()", source)
        self.assertIn("Math.random() * 1001", source)
        ...
        self.assertIn('" 질문 보기"', source)
        self.assertIn("function renderQuestionPanel()", source)
        self.assertIn('className: "brain-question-panel"', source)
        self.assertIn('className: "brain-question-section-add"', source)
        self.assertIn('title: section.title + "에 메모 추가"', source)
        self.assertIn('h("span", null, "메모 추가")', source)
        self.assertNotIn('" 메모 작성"', source)
        self.assertNotIn("editor.referenceQuestion", source)
        self.assertIn("slotInRegion(targetIndex, targetTaken.length, targetTaken)", source)
        self.assertIn('className: "brain-assignee-menu"', source)
        self.assertIn('"담당자 지정"', source)
        self.assertIn('apiBase + "nodes/batch-position/"', source)
        self.assertIn("event.shiftKey", source)
        self.assertIn('historyAction("undo/"', source)
        self.assertIn('historyAction("redo/"', source)
        ...
        self.assertIn(".brain-question-panel", css)
        self.assertIn(".brain-note-actions{min-width:270px;flex-wrap:nowrap}", css)
        self.assertIn(".brain-note.multi-selected", css)
        self.assertIn(".brain-multi-banner", css)
        ...
        self.assertIn("var CANVAS_W = 6200", layout)
        self.assertIn("var CANVAS_H = 3600", layout)
        self.assertIn("function isDraggingNode(nodeId)", source)
        self.assertIn("Array.isArray(dragging) ? dragging.includes(nodeId)", source)
        self.assertIn("미분류 메모는 6200×3600 자유 캔버스 어디에나 둘 수 있다", source)
        self.assertNotIn("var trayTaken = [];", source)
        self.assertIn("function nearestFreeNotePosition(wantX, wantY)", source)
        self.assertIn("for (var ring = 1; ring <= 24; ring += 1)", source)
        self.assertIn("settleInRegion(targetIndex, x, y, targetTaken)", source)
        self.assertIn("function (created) { setFocused(created.id); }", source)
        ...
        self.assertIn("var DEFAULT_CANVAS_ZOOM = .55", source)
        self.assertIn("fitBoardView(DEFAULT_CANVAS_ZOOM, DEFAULT_CANVAS_ZOOM)", source)
        self.assertNotIn("fitBoardView(.5, .5)", source)
        self.assertIn('"ver." + node.introduced_in_version', source)
        self.assertIn("function clearFocusedNode(event)", source)
        self.assertIn("editor.node ? editor.node.author_id : state.current_user_id", source)
    ...
    def test_prd_write_screen_exposes_inline_participant_management(self):
        ...
        self.assertIn('id="answer-held-conflict-modal"', template)
        self.assertIn("질문이 보류되었습니다", template)
        self.assertIn('id="answer-held-conflict-copy"', template)
        self.assertIn("if (latestQuestion.is_held)", script)
        self.assertIn("pendingAnswers.delete(questionId)", script)
        self.assertIn("navigator.clipboard.writeText(answerHeldConflictLocal.value)", script)
