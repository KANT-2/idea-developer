...
## 2. 세 관점 통합 PRD 전체 초안과 종합 진단

`AI 진단하기`는 PM, 엔지니어링, 투자자 관점을 각각 평가하고 세 결과가 모두 최신이면 별도
종합 job이 관점별 중요도를 판단한 종합 점수와 의견을 만든다. `AI 초안 작성`은 관점 하나를
선택하지 않고 세 관점을 함께 반영해 PRD 전체 질문의 통합 답변 초안을 생성한다.

처리 흐름은 다음과 같다.

1. 로그인·PRD 접근·AI 요청 권한을 서버에서 검증한다.
...
현재 질문 version과 일치하는 같은 사용자의 관점별 최신 진단 결과가 있으면 초안의 참고자료로
사용한다. 일부 또는 모든 진단 결과가 없거나 오래됐어도 초안 요청 자체를 막지 않는다. 전체 PRD를
대상으로 하므로 job timeout은 60초다.
...
갱신한다. `0022`~`0027`은 통합 초안 prompt를 버전 관리하며, `0028`~`0029`는
`PRD_EVALUATION_SYNTHESIS/synthesis` 조합과 종합 prompt를 등록한다. 기존 작업·사용 로그 행을
삭제하거나 덮어쓰지 않는다.

추가 API는 다음과 같다.

- `GET .../ai/evaluation/`: 세 관점과 종합 job의 최신 상태 조회
- `POST .../ai/evaluation/synthesis/run/`: 최신 세 관점 결과의 종합 job 생성
- `POST .../ai/perspective-draft/run/`: 세 관점 통합 초안 job 생성
...
- 통합 초안에서도 질문·초안을 해당 섹션 코치 입력란에 전달할 수 있다.
...
- 활성 질문 없음, 빈·누락·중복·외부 질문 ID는 저장 전에 거절한다.
- 종합 진단은 세 관점이 모두 성공하고 현재 질문 version과 일치할 때만 생성하며, 반환 섹션 ID와
  상태값을 서버에서 다시 검증한다.
...
- `tests/test_ai_perspective_draft.py`가 세 persona 입력, 멱등성, 60초 timeout 전달, 출력 ID 검증,
...
- `tests/test_ai_prd_evaluation.py`가 세 관점 완료 전 종합 요청 거절, 종합 job 멱등성, worker 처리와
  최신 결과 복원을 검증한다.

배포 전에는 `python manage.py migrate`로 관련 migration을 적용하고 AI worker가 동일한 최신 코드를
