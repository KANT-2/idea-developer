...
| PRD 작성 화면 | `hyungjune` |
| 테스트 | `sungho` |
| 예비 브레인스토밍 브랜치 | `nakyoung` |

각 브랜치는 작업 전 최신 `develop`을 병합하고 작업 후 `develop` 대상 PR을 만듭니다. 담당자가
없는 영역이나 공통 충돌 해결은 팀장 브랜치에서 수행합니다. 아래 기능형 브랜치 이름은 향후 작업을
더 세분화할 때 사용할 수 있는 권장 예시입니다.
