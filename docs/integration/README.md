...
외부 시스템이 제공하는 PostgreSQL VIEW 규격과 애플리케이션 연동 원칙을 설명합니다.
...
- 진행 중 회차의 실제 status 문자열은 연동 대상 시스템의 값을 확인해 `INTEGRATION_ACTIVE_ROUND_STATUSES`로 설정합니다.
...
- `VIEW_GUIDE.md`: PostgreSQL VIEW 필드와 읽기 전용 연동 규격
