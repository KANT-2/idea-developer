...
| AI 진단·종합 | 한 번의 실행으로 PM·엔지니어링·투자자 진단을 만들고, 세 결과가 최신이면 별도 AI job으로 단순 평균이 아닌 종합 점수와 의견을 생성 |
| 세 관점 통합 PRD 초안 | 세 관점을 함께 반영한 전체 활성 질문의 답변 초안을 비동기 생성하며, 미리보기에서 선택 승인한 질문만 version 재검증 후 반영 |
| 브레인스토밍 분류 결과 | AI 호출 없이 서버의 현재 섹션·상태 통계를 표시 |
| 메모 상태 | 미분류는 default, 섹션 배치 시 accepted, 보류 구역은 held |
| 보드 버전 | PRD마다 여러 버전을 두고 `display_order` 첫 보드를 최신으로 사용한다. 최신만 편집 가능하며 과거 버전은 조회 전용이고, 순서 변경으로 최신을 지정하며 최신 삭제 시 다음 보드를 승격한다. |
| 캔버스 조작 | Shift 다중 선택·일괄 이동, Ctrl/Cmd+Z 실행 취소, Ctrl/Cmd+Y 다시 실행, Esc 선택 해제를 지원하고 서버 version 충돌 시 덮어쓰지 않는다. |
...
| 알림 | PRD 참여자 추가 시 해당 PRD 바로가기 URL을 포함하고, 참여자 추가와 코멘트 생성 시 부모 Slack 공통 함수를 DB commit 이후 호출하며, 별도 비동기 큐 없이 일시적 실패를 최대 3회 재시도하고 최종 실패가 본 기능 저장을 취소하지 않게 처리 |
| 최근 활동 | 접근 가능한 PRD의 변경만 보여주며 같은 사용자·PRD·이벤트의 30분 내 반복은 화면에서 묶고 상세 변경 이력은 삭제하지 않는다. |
| 운영 로그 | JSON 로그에 request ID와 호출부 `extra` 필드를 함께 보존하고 직렬화 불가능한 값은 문자열로 안전하게 기록 |
