...
> **보관용 구현 이력 — 현재 요구사항 문서가 아닙니다.**
>
> 이 문서에는 개발 당시의 단계별 지시, `아직 구현하지 마`와 같은 시점 의존 문장, 이후 폐기된
> 정책이 포함되어 있습니다. 신규 구현이나 코드 리뷰의 근거로 직접 사용하지 마십시오. 현재
> 정책은 `docs/requirements/CURRENT_REQUIREMENTS.md`를 최우선으로 하고, 상세 규칙은 같은 폴더의
> 홈·브레인스토밍 정책 시나리오를 따릅니다.

이 문서는 `idea-developer`를 독립 Django 시스템으로 구축하기 위한 프롬프트 모음이다. 우리 팀은 단독 시스템까지만 완성하고, 부모 프로젝트로의 실제 이식은 부모 운영 팀이 담당한다.
...
```text
이 프로젝트는 독립 실행 가능한 `idea-developer` 시스템으로 만든다.
