...
이 문서는 홈 화면, PRD 생성·조회, KPI, 필터, 권한 및 부모 연동에 관한 제품 시나리오와
서버 계약을 정리한다. 초기 UI 목업과 첨부 자료는 요구사항을 추출하기 위한 참고 자료였으며,
현재 구현 여부는 각 절의 명시적인 상태와 최우선 정책 문서를 기준으로 판단한다.
...
PRD 접근 범위는 현재 회차로 일괄 제한하지 않는다. 현재 또는 과거 회차 PRD와 회차 없는 PRD
모두 명시적인 PRD 참여 관계를 기준으로 노출한다. 회차·프로젝트 구분은 서버 필터로 제공한다.
회차 팀 PRD의 쓰기 작업만 해당 `user_id + round_id` 참가정보와 `team_id`를 추가 검증한다.

`전체 PRD`와 목록의 전체 탭이 서로 다른 범위를 사용하지 않도록, 서버에서 공통 조회 조건을 한 번 정의해 재사용한다.

### 부모 시스템과 실행 모드

`idea-developer`는 우리 팀이 독립 Django 시스템으로 완성해 전달하는 프로젝트다. 부모 시스템으로 실제 이식하는 작업은 부모 프로젝트 운영 팀이 담당한다. 우리 구현 범위는 단독 실행 가능한 시스템이며, 이식하기 쉽도록 스택·외부 식별자·VIEW 조회 규격·URL namespace·템플릿 계약을 맞춘다.
...
부모 저장소를 직접 수정하거나 임시 이식본에 의존하지 않고, 호환 가능한 독립 시스템을 만드는 참고 자료로만 사용한다.
...
`idea-developer` MVP도 먼저 이 운영 패턴에 맞춘다. Redis·Celery·Channels가 꼭 필요한 기능은 부모 팀이 이식할 때 추가 도입을 판단할 수 있도록 별도 선택사항으로 문서화하되, 우리 단독 시스템의 필수 의존성으로 만들지 않는다.
...
현재 회차 문맥이 필요한 기능은 VIEW에서 회차 상태와 사용자 참가 정보를 조회한다. 진행 중 회차가
하나면 기본 선택하고, 여러 개면 사용자가 선택한 값을 다시 검증한다. 진행 중 회차가 없어도 홈,
회차 없는 PRD 생성과 명시적 참여 PRD 조회는 사용할 수 있다. 임의의 최신 회차를 현재 회차로
선택하지 않는다.
...
자식 시스템의 PRD에는 다음 외부 식별자를 저장한다.

- `round_id`: 회차 PRD에서는 필수, 회차 없는 PRD에서는 `null`
- `team_id`: 회차 팀 또는 일반 팀 PRD에서 사용하며 개인 PRD에서는 `null`
- 생성자의 `user_id`: 필수
...
상단 인사에는 로그인 사용자의 표시 이름을 사용한다. 현재 회차 snapshot이 있으면 우선 사용하고,
없어도 사용자 VIEW의 이름 또는 이메일 폴백으로 화면을 정상 표시한다.
...
참여자 선택 영역에는 `현재 내가 속한 팀`과 `개별 추가`를 제공한다. 회차 정보가 없으면 현재 팀
불러오기는 빈 상태로 표시하되 개별 사용자 검색과 명시적 참여자 추가는 계속 허용한다.

- `현재 내가 속한 팀`에는 회차 문맥이 있을 때 로그인 사용자가 해당 회차에 속한 팀만 반환한다.
...
팀원 검색은 전체 사용자 정보를 내려받아 프론트에서 찾지 않고 서버 검색 API를 사용한다. 회차
PRD에서는 해당 회차 참가자로 제한하고 필요하면 해당 팀으로 추가 제한한다. 회차 없는 PRD에서는
활성·승인 사용자 VIEW 범위에서 검색하되 페이지네이션과 최소 검색 길이를 적용한다.
...
PRD 생성 요청의 참여자 ID는 서버에서 다시 검증한다. 존재하지 않거나 비활성화된 사용자,
추가 권한이 없는 사용자, 중복 ID는 그대로 저장하지 않는다.

회차 PRD의 참여자 검증은 `user_round_team_view`의 `participant_id`, `round_id`, `team_id`를
기준으로 한다. 회차 없는 PRD는 활성·승인 사용자 VIEW와 명시적인 PRD 참여 관계를 사용한다.

참여자 초대 수락 단계는 두지 않는다. 권한 있는 사용자가 추가하면 참여 관계가 즉시 생성되며,
추가된 사용자는 Slack 알림과 최근 활동으로 이를 확인한다. `(prd_id, user_id)` 유니크 제약으로
팀 전체 추가, 개별 추가와 네트워크 재시도에서 중복 관계가 생기지 않게 한다.
...
`프로젝트` 탭은 `prd_type = new_product` 필터로 확정한다. 팀 공유 여부는 별도의 `팀` 탭과
프로젝트 구분 필터에서 처리한다.
...
- 보류된 질문은 분모와 분자에서 제외하며 AI PRD 충족도 입력에서도 제외한다.
- 질문이 하나도 없으면 `0%`다.
...
다음 항목만 현재 범위에서 미결정으로 유지한다.

- 공유 권한 뱃지를 홈 카드에 추가로 노출할지
...
표시하고, 더보기 모달에서 서버 페이지네이션으로 전체 이력을 조회한다. 같은 PRD에서 같은
사용자가 30분 안에 반복한 같은 이벤트는 사용자 화면에서 한 항목과 건수로 묶는다. 감사와
동시 수정 추적에 사용하는 원본 `PrdChangeHistory` 행은 합치거나 삭제하지 않는다.
...
- 참여자는 초대 수락 없이 즉시 추가한다.
- 비활성·미승인 사용자는 추가하지 않는다.
- 동명이인은 마스킹한 이메일 일부와 필요한 최소 팀 정보로 구분한다.
- 참여자 추가와 코멘트 생성은 최근 활동에 기록하고 부모 공통 Slack DM 함수를 호출한다.
- 이메일 초대 알림은 현재 범위에 포함하지 않는다.
...
PRD 생성 자체와 알림 전송을 분리한다. PRD와 참여 관계 저장이 성공했다면 생성 요청을 실패로
되돌리지 않는다. DB commit 이후 부모의 `notifications.slack` 공통 함수를 호출하며 일시적 예외는
짧은 지수형 대기로 최대 3회 재시도한다. 별도 Slack 작업 큐는 두지 않으며 최종 실패는 서버
로그에 남기되 참여자 추가나 코멘트 저장을 취소하지 않는다.

### 부모 탭에서 회차 정보가 전달되지 않은 경우

임의로 최신 회차를 선택하지 않는다. 회차가 필요한 회차 팀 기능은 회차 선택 또는 재진입을
요청하지만, 홈 조회와 회차 없는 개인·일반 팀 PRD 생성은 계속 허용한다.
...
회차 팀 PRD의 생성·수정에서는 해당 PRD의 `round_id`에 해당하는 팀만 사용한다. 홈에서는
명시적 참여 관계가 있는 과거 PRD도 노출하되, 카드와 필터에 PRD의 실제 회차·프로젝트 구분을
그대로 표시한다. 서로 다른 회차의 팀원을 한 PRD의 현재 팀으로 합치지 않는다.
